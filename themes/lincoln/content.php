...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="featured-post">
			<?php _e( 'Featured post', 'lincolncolllective' ); ?>
		</div>
		<?php endif; ?>
		...
		<footer class="entry-meta">
			<?php lincolncolllective_entry_meta(); ?>
			...
			<?php if ( is_singular() && get_the_author_meta( 'description' ) && is_multi_author() ) : ?>
				<div class="author-info">
					<div class="author-avatar">
						...
						$author_bio_avatar_size = apply_filters( 'lincolncolllective_author_bio_avatar_size', 68 );
						echo get_avatar( get_the_author_meta( 'user_email' ), $author_bio_avatar_size );
						?>
					</div>
					<div class="author-description">
						<h2><?php printf( __( 'About %s', 'lincolncolllective' ), get_the_author() ); ?></h2>
						<p><?php the_author_meta( 'description' ); ?></p>
						<div class="author-link">
							<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
								<?php printf( __( 'View all posts by %s <span class="meta-nav">&rarr;</span>', 'lincolncolllective' ), get_the_author() ); ?>
							</a>
						</div>
					</div>
				</div>
